...
export function useTreeExpansion(
  ...
  // Initialize expanded nodes with auto-expansion logic
  const initialExpandedNodes = useMemo(() => {
    ...
    if (persistKey && typeof window !== 'undefined') {
      try {
        ...
        if (saved) {
          ...
          savedPaths.forEach(path => expanded.add(path));
          return expanded;
        }
      } catch (error) {
        console.warn('Failed to load tree expansion state from localStorage:', error);
      }
    }

    // Auto-expand based on level
    const collectAutoExpandNodes = (node: TreeNode, level: number) => {
      if (shouldAutoExpand(level, maxAutoExpandLevel) && node.children?.length) {
        expanded.add(node.path);
      }
      node.children?.forEach(child => collectAutoExpandNodes(child, level + 1));
    };
    ...
    return expanded;
  }, [tree, maxAutoExpandLevel, persistKey]);
  ...
  const persistExpansionState = useCallback((newExpandedNodes: Set<string>) => {
    if (persistKey && typeof window !== 'undefined') {
      try {
        ...
        console.warn('Failed to save tree expansion state to localStorage:', error);
      }
    }
  }, [persistKey]);

  const isExpanded = useCallback((nodePath: string, level: number): boolean => {
    return expandedNodes.has(nodePath);
  }, [expandedNodes]);

  const toggleNode = useCallback((nodePath: string, level: number) => {
  ...
  const setExpanded = useCallback((nodePath: string, expanded: boolean) => {
    setExpandedNodes(prev => {
      const newSet = new Set(prev);
      if (expanded) {
        newSet.add(nodePath);
